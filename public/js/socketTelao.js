...
function atualizar(pedidos) {
    ...
    pedidos.forEach(pedido => {
        if(pedido.andamento == 0){
            aguardo.innerHTML += `<h3 class="text-xl text-white">${pedido.nome} | ID ${pedido.id}</h3>`;
        }else if(pedido.andamento == 1){
            andamento.innerHTML += `<h3 class="text-xl text-white">${pedido.nome} | ID ${pedido.id}</h3>`;
        }else{
            finalizado.innerHTML += `<h3 class="text-xl text-white">${pedido.nome} | ID ${pedido.id}</h3>`;
        }
    })
}
